...
export function DocumentAnalyzerGenerator() {
  ...
  const handleReset = () => {
    ...
    setDocType("article");
    setFileName("");
    setSelectedOptions(["summary", "insights"]);
    setReadingLevel("general");
    setLengthPref("balanced");
    setFocusAreas("");
    setShowAdvanced(false);
    setExtractStats(true);
    setExtractQuotes(true);
    setStreamingText("");
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (file.type === "application/pdf" || file.name.endsWith(".pdf")) {
      try {
        ...
        if (trimmed.length < 50) {
          setPdfError(
            "Could not extract enough text from this PDF. It may be image-based or scanned. Try pasting the text manually.",
          );
        } else {
    ...
    setPdfError(
      `Unsupported file type: ${file.type || file.name.split(".").pop()}. Please use PDF or TXT files, or paste your text directly.`,
    );
    setIsLoadingPdf(false);
  };
...
Q: `,
    ...
    const result = await generateRaw({
      ...
      temperature: 0.6,
      maxTokens:
        sectionType === "summary" || sectionType === "simplified" ? 800 : 600,
  ...
  const handleGenerate = async () => {
    ...
    const levelLabel =
      READING_LEVELS.find((l) => l.value === readingLevel)?.label || "General";
    const lengthLabel =
      LENGTH_PREFS.find((l) => l.value === lengthPref)?.label || "Balanced";

    const sectionsToGenerate: string[] = [...selectedOptions];
    if (extractStats && !sectionsToGenerate.includes("stats"))
      sectionsToGenerate.push("stats");
    if (extractQuotes && !sectionsToGenerate.includes("quotes"))
      sectionsToGenerate.push("quotes");
    ...
    try {
      for (let i = 0; i < sectionsToGenerate.length; i++) {
        ...
        setGenerationProgress(
          `Generating ${label}... (${i + 1}/${sectionsToGenerate.length})`,
        );
      ...
      const analysis: DocumentAnalysis = {
        ...
        studyQuestions: results.questions
          ? parseQuestionList(results.questions)
          : [],
  ...
  const buildFullText = () => {
    ...
    if (!hasStructured && currentAnalysis.rawText)
      return currentAnalysis.rawText;
    const parts: string[] = [];
    if (currentAnalysis.summary) {
      parts.push("SUMMARY:");
      parts.push(currentAnalysis.summary);
      parts.push("");
    }
    if (currentAnalysis.keyInsights.length > 0) {
      parts.push("KEY INSIGHTS:");
      currentAnalysis.keyInsights.forEach((i) => parts.push(`- ${i}`));
      parts.push("");
    }
    if (currentAnalysis.simplifiedExplanation) {
      parts.push("SIMPLIFIED EXPLANATION:");
      parts.push(currentAnalysis.simplifiedExplanation);
      parts.push("");
    }
    if (currentAnalysis.importantQuotes.length > 0) {
      parts.push("IMPORTANT QUOTES:");
      currentAnalysis.importantQuotes.forEach((q) => parts.push(`"${q}"`));
      parts.push("");
    }
    if (currentAnalysis.keyStats.length > 0) {
      parts.push("KEY STATISTICS:");
      currentAnalysis.keyStats.forEach((s) => parts.push(`- ${s}`));
      parts.push("");
    }
    if (currentAnalysis.actionItems.length > 0) {
      parts.push("ACTION ITEMS:");
      currentAnalysis.actionItems.forEach((a) => parts.push(`- ${a}`));
      parts.push("");
    }
    if (currentAnalysis.studyQuestions.length > 0) {
      parts.push("STUDY QUESTIONS:");
      currentAnalysis.studyQuestions.forEach((q) => parts.push(`Q: ${q}`));
      parts.push("");
    }
    return parts.join("\n");
  };
  ...
  return (
    <div
      ...
      data-testid="container-document-analyzer"
    >
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-8">
        ...
        <div className="space-y-5">
          <div>
            ...
            <div
              ...
              className={cn(
                ...
                fileName
                  ...
                  : "border-slate-200 bg-slate-50/30 hover:border-purple-300",
              )}
              ...
              {isLoadingPdf ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Supports .pdf and .txt (max ~50 pages)
                  </p>
                </div>
              )}
            </div>
            {pdfError && (
              <div
                className="mt-2 p-3 rounded-lg bg-amber-50 border border-amber-200 flex items-start gap-2"
                data-testid="text-pdf-error"
              >
          ...
          <div>
            ...
            <textarea
              ...
              value={documentText}
              onChange={(e) =>
                setDocumentText(e.target.value.slice(0, MAX_CHARS))
              }
            ...
            <div className="flex justify-between mt-1">
              <span className="text-xs text-slate-400">
                {documentText.length > 0
                  ...
                  : "Paste or upload a document to analyze"}
              </span>
              <span
          ...
          <div>
            ...
            <div
              ...
              data-testid="container-doc-type"
            >
              {DOC_TYPES.map((dt) => (
                <button
                  key={dt.value}
                  data-testid={`toggle-doctype-${dt.value}`}
          ...
          <div>
            ...
            <div
              ...
              data-testid="container-analysis-options"
            >
              ...
              {showAdvanced && (
                ...
                >
                  <div className="pt-3 space-y-3">
                    <ToggleOption
                      testId="toggle-extract-stats"
                      label="Extract all numbers & statistics"
                      value={extractStats}
                      onChange={() => setExtractStats(!extractStats)}
                    />
                    <ToggleOption
                      testId="toggle-extract-quotes"
                      label="Highlight important quotes"
                      value={extractQuotes}
                      onChange={() => setExtractQuotes(!extractQuotes)}
                    />
                  </div>
                </motion.div>
              )}
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-medium text-purple-600">
              {generationProgress || "Analyzing document... 100% in-browser"}
            </span>
          </div>
          <pre
            className="whitespace-pre-wrap text-sm text-slate-600 font-mono leading-relaxed"
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              ...
              <p
                ...
                data-testid="text-result-subtitle"
              >
                {currentAnalysis.documentTitle} |{" "}
                {currentAnalysis.charCount.toLocaleString()} chars analyzed
              </p>
            </div>
          ...
          >
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-600">
              ...
              {Math.ceil(currentAnalysis.charCount / 4000)} pages (est.)
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-600">
          ...
          {currentAnalysis.keyInsights.length > 0 && (
            <div
              ...
              data-testid="container-insights"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Lightbulb className="w-5 h-5 text-purple-500" />
                  <h3 className="text-base font-bold text-slate-800">
                    Key Insights ({currentAnalysis.keyInsights.length})
                  </h3>
                </div>
                <CopyBtn
                  id="insights"
                  ...
                  onClick={() =>
                    copyToClipboard(
                      currentAnalysis.keyInsights.join("\n"),
                      "insights",
                    )
                  }
                />
          ...
          {currentAnalysis.simplifiedExplanation && (
            <ResultSection
              ...
              icon={BookOpen}
              testId="container-simplified"
              text={currentAnalysis.simplifiedExplanation}
              copiedId={copiedId}
              onCopy={(text) => copyToClipboard(text, "simplified")}
              copyId="simplified"
            />
          )}
          ...
          {currentAnalysis.importantQuotes.length > 0 && (
            <div
              className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-6"
              data-testid="container-quotes"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Quote className="w-5 h-5 text-purple-500" />
                  <h3 className="text-base font-bold text-slate-800">
                    Important Quotes ({currentAnalysis.importantQuotes.length})
                  </h3>
                </div>
                <CopyBtn
                  id="quotes"
                  ...
                  onClick={() =>
                    copyToClipboard(
                      currentAnalysis.importantQuotes
                        .map((q) => `"${q}"`)
                        .join("\n"),
                      "quotes",
                    )
              ...
              <div className="space-y-3">
                {currentAnalysis.importantQuotes.map((quote, i) => (
                  <div
                    key={i}
                    className="p-3 rounded-xl border-l-4 border-purple-300 bg-purple-50/30 text-sm italic text-slate-700"
                    data-testid={`quote-${i}`}
                  >
                    "{quote}"
                  </div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-bold text-slate-800">
                    Key Statistics ({currentAnalysis.keyStats.length})
                  </h3>
                </div>
                <CopyBtn
                  id="stats"
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-bold text-slate-800">
                    Action Items ({currentAnalysis.actionItems.length})
                  </h3>
                </div>
                <CopyBtn
                  id="actions"
                  ...
                  onClick={() =>
                    copyToClipboard(
                      currentAnalysis.actionItems.join("\n"),
                      "actions",
                    )
                  }
                />
          ...
          {currentAnalysis.studyQuestions.length > 0 && (
            <div
              className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-6"
              data-testid="container-questions"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <HelpCircle className="w-5 h-5 text-purple-500" />
                  <h3 className="text-base font-bold text-slate-800">
                    Study Questions ({currentAnalysis.studyQuestions.length})
                  </h3>
                </div>
                <CopyBtn
                  id="questions"
                  ...
                  onClick={() =>
                    copyToClipboard(
                      currentAnalysis.studyQuestions
                        .map((q) => `Q: ${q}`)
                        .join("\n"),
                      "questions",
                    )
          ...
          {!currentAnalysis.summary &&
            ...
            currentAnalysis.rawText && (
              <div
                className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-6"
                data-testid="container-raw-analysis"
              >
                <div className="flex items-center gap-2 mb-4">
                  <FileText className="w-5 h-5 text-purple-500" />
                  <h3 className="text-base font-bold text-slate-800">
...
function EngineStatus({
  ...
  return (
    <div
      className="mb-6 p-4 rounded-xl bg-purple-50 border border-purple-200"
...
function formatRawText(raw: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      if (heading)
        elements.push(
          <h4 key={`h-${i}`} className="font-bold text-slate-800 mt-3 mb-1">
            {heading}
          </h4>,
        );
      ...
      elements.push(
        <div
          key={`li-${i}`}
          className="flex items-start gap-2 text-sm text-slate-700 ml-2"
        >
      ...
      elements.push(
        <div
          key={`q-${i}`}
          className="p-2 rounded-lg border-l-4 border-purple-300 bg-purple-50/30 text-sm italic text-slate-700 ml-2"
        >
          {trimmed.replace(/^>>?\s*/, "")}
        </div>,
      );
      ...
      elements.push(
        <p key={`q-${i}`} className="text-sm font-semibold text-slate-800">
          {trimmed}
        </p>,
      );
    } else {
