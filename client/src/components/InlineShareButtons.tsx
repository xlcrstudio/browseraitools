...
export function InlineShareButtons({
  ...
  const toolName =
    typeof document !== "undefined"
      ? document
          .querySelector<HTMLElement>('[data-testid="text-tool-title"]')
          ?.textContent?.trim() ||
  ...
  const iconBtn =
    "w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 dark:hover:bg-slate-700";

  return (
    <div
      className={cn("flex items-center gap-0.5", className)}
      data-testid="inline-share-buttons"
    >
